...
space_similarity <-
  function(
    ...
    if (method %in% ml_methods) {
      ...
      caret::checkInstall(
        caret::getModelInfo(method, regex = FALSE)[[1]]$library
      )
    }
    ...
    if (
      min(sapply(split_data_list, nrow)) <= length(dimensions) &
        method == "probability"
    ) {
      stop2(
    ...
    if (method %in% c("density.overlap", "mean.density.overlap")) {
      total_coors <-
        spatstat.geom::as.ppp(
          as.matrix(data[, dimensions]),
          c(range(data[, dimensions[1]]), range(data[, dimensions[2]]))
        )
      total_space <-
      ...
      input_data <-
        lapply(split_data_list, function(Y, dims = dimensions) {
          coors <-
            spatstat.geom::as.ppp(
              as.matrix(Y[, dimensions]),
              c(range(Y[, dimensions[1]]), range(Y[, dimensions[2]]))
            )
          ...
          raster::values(raster_dens) <-
            (raster::values(raster_dens) -
              min(raster::values(raster_dens), na.rm = TRUE))
          raster::values(raster_dens) <-
            raster::values(raster_dens) /
            max(raster::values(raster_dens), na.rm = TRUE)

          # keep 95% interval
          raster::values(raster_dens)[
            raster::values(raster_dens) < 1 - outliers
          ] <-
            NA
    ...
    ovlp_fun <- function(W, Z, WZ, tp, dims, dist.meth, seed) {
      ...
      if (tp %in% c("density.overlap", "mean.density.overlap")) {
        ...
        wgt_ovlp.WvZ <-
          sum(
            (raster::values(W) * raster::values(Z)) / raster::values(Z),
            na.rm = TRUE
          ) /
          sum(raster::values(Z), na.rm = TRUE)

        # 2 vs 1
        wgt_ovlp.ZvW <-
          sum(
            (raster::values(Z) * raster::values(W)) / raster::values(W),
            na.rm = TRUE
          ) /
          sum(raster::values(W), na.rm = TRUE)
        ...
        out <- matrix(
          c(wgt_ovlp.WvZ, wgt_ovlp.ZvW, mean(c(wgt_ovlp.WvZ, wgt_ovlp.ZvW))),
          nrow = 1
        )
      }
      ...
      if (tp %in% c("mcp.overlap", "mean.mcp.overlap")) {
        ...
        out <- matrix(
          c(ovlp2in1, ovlp1in2, mean(c(ovlp2in1, ovlp1in2))),
          nrow = 1
        )
      }
      ...
      if (tp %in% ml_methods) {
        ...
        out <- matrix(
          1 - train_caret$results$Accuracy[nrow(train_caret$results)],
          nrow = 1
        )
        ...
        if (!is.null(train_caret$finalModel$confusion)) {
          ...
          out <- matrix(
            c(
              conf_matrix[rownames(conf_matrix) == W[1, group], 3],
              conf_matrix[rownames(conf_matrix) == Z[1, group], 3],
              1 - train_caret$results$Accuracy[nrow(train_caret$results)]
            ),
            nrow = 1
          )
      ...
      if (tp == "probability") {
        wz.par <- tapply(1:nrow(WZ), WZ[, group], function(ii) {
          nicheROVER::niw.post(nsamples = 1000, X = WZ[ii, dims])
        })

        # overlap calculation
        over <- nicheROVER::overlap(
          wz.par,
          nreps = 1000,
          nprob = 1000,
          alpha = c(outliers)
